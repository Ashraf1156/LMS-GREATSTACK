...
const getYouTubeVideoId = (url) => {
    ...
    try {
        ...
        else if (url.length === 11 && !url.includes('.') && !url.includes('/')) { videoId = url; }
    } catch (error) { console.error("Error parsing YouTube URL:", url, error); return null; }
...
const CourseDetails = () => {
    ...
    const enrollCourse = async () => {
        ...
        if (!userData) { toast.warn('Login to Enroll'); return; }
        if (isAlreadyEnrolled) { navigate(`/player/${courseData._id}`); toast.info('Already Enrolled, redirecting...'); return; }

        try {
            const token = await getToken();
            if (!token) { toast.error("Authentication token unavailable."); return; }
    ...
    useEffect(() => {
        if (!isUserDataLoading && userData && courseData) {
            setIsAlreadyEnrolled(userData.enrolledCourses?.includes(courseData._id) || false);
        } else if (!isUserDataLoading && !userData) {
            setIsAlreadyEnrolled(false);
        }
    }, [isUserDataLoading, userData, courseData]);

    if (isLoadingCourse || (!courseData && !isLoadingCourse)) {
        return <Loading /> || <div className="min-h-screen flex items-center justify-center">Loading course...</div>;
    }

    // Use safe calculation for rating
        ...
        <>
            <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-20 pt-10 text-left min-h-screen">
                ...
                <div className="max-w-xl z-10 text-gray-500">
                    ...
                    <p className="pt-4 md:text-base text-sm line-clamp-3" dangerouslySetInnerHTML={{
                    ...
                    <div className='flex items-center flex-wrap space-x-2 pt-3 pb-1 text-sm'>
                        ...
                        <div className='flex mr-1'>
                            {[...Array(5)].map((_, i) =>
                                (<img key={i}
                                      src={i < Math.round(Number(safeRating)) ? assets.star : assets.star_blank}
                    ...
                    <div className="pt-8 text-gray-800">
                        ...
                        <div className="pt-5">
                                ...
                                <div key={chapter.chapterId || index} className="border border-gray-300 bg-white mb-2 rounded">
                                    <div className="flex items-center justify-between px-4 py-3 cursor-pointer select-none hover:bg-gray-50"
                                        ...
                                        <div className="flex items-center gap-2 overflow-hidden mr-2">
                                            <img src={assets.down_arrow_icon} alt="Toggle section"
                                                 className={`w-4 h-4 transform transition-transform flex-shrink-0 ${openSections[index] ? "rotate-180" : ""}`} />
                                            <p className="font-medium md:text-base text-sm truncate">{index + 1}. {chapter.chapterTitle}</p>
                                        </div>
                                        <p className="text-xs md:text-sm text-gray-500 flex-shrink-0 whitespace-nowrap">
                                            {chapter.chapterContent?.length || 0} {(chapter.chapterContent?.length === 1 ? 'lec' : 'lecs')} - {calculateChapterTime(chapter)}
                                        </p>
                                    </div>
                                    <div className={`overflow-hidden transition-all duration-300 ${openSections[index] ? "max-h-screen border-t" : "max-h-0"}`}>
                                        <ul className="py-2 text-gray-600">
                                            {(chapter.chapterContent || []).map((lecture, i) => (
                                                <li key={lecture.lectureId || i}
                                                    className="flex items-start gap-2 py-1.5 px-4 hover:bg-gray-100">
                                                    ...
                                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between w-full text-xs md:text-sm">
                                                        ...
                                                        <div className='flex gap-2 items-center flex-shrink-0 mt-1 sm:mt-0'>
                                                            {lecture.isPreviewFree && lecture.lectureUrl && (
                                                                <button onClick={() => handlePreviewClick(lecture)}
                                                                        className='text-blue-600 hover:text-blue-800 text-xs font-medium'>
                                                                    Preview
                                                                </button>
                                                            )}
                                                            <span className='text-gray-500'>{lecture.lectureDuration
                            ...
                            {(!courseData.courseContent || courseData.courseContent.length === 0) && (
                                <p className="text-gray-500 pt-2">Course content coming soon.</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="py-10 md:py-16 text-sm md:text-default">
                        ...
                        <div className="rich-text pt-3" dangerouslySetInnerHTML={{
                    ...
                    <div className="aspect-video bg-gray-200">
                        { playerData
                            ? <YouTube videoId={playerData.videoId} opts={{ playerVars: { autoplay: 1 } }}
                                       ...
                                       onError={(e) => { console.error("YouTube Player Error:", e); toast.error("Error loading preview video."); setPlayerData(null); }} />
                            : <img className='w-full h-full object-cover'
                                   src={courseData.courseThumbnail || 'placeholder.png'}
                    ...
                    <div className="p-5">
                        <div className="flex items-center justify-between text-sm md:text-default gap-4 pt-2 pb-4 text-gray-500 border-b mb-4">
                            ...
                            <div className="flex items-center gap-1" title="Number of lessons">
                                ...
                                <span>{calculateNoOfLectures(courseData)} {calculateNoOfLectures(courseData) === 1 ? 'lesson' : 'lessons'}</span>
                            </div>
                        </div>
                        <button
