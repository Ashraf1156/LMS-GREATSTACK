...
const CourseDetails = () => {
    ...
    const fetchCourseData = async () => {
        ...
        try {
            const { data } = await axios.get(`${backendUrl}/api/course/${id}`); // Template literal
            if (data.success) {
            ...
            toast.error(`Failed to load course: ${error.message}`); navigate('/');
        } finally {
    ...
    const enrollCourse = async () => {
        ...
        if (!userData) { toast.warn('Login to Enroll'); return; } // Consider triggering Clerk's sign in modal
        ...
        try {
            ...
            if (data.success) {
                ...
                setUserData(prev => ({ ...prev, enrolledCourses: [...(prev?.enrolledCourses || []), courseData._id] }));
                setIsAlreadyEnrolled(true);
                navigate(`/player/${courseData._id}`);
            } else {
                 // Handle specific backend messages like 'Already Enrolled'
                 if (data.message === 'Already Enrolled') {
                      setIsAlreadyEnrolled(true); // Sync state
                      toast.info(data.message);
                      navigate(`/player/${courseData._id}`); // Redirect if already enrolled
                 } else {
    ...
    useEffect(() => { setPlayerData(null); fetchCourseData(); }, [id, backendUrl]);
            ...
            <div className="flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-20 pt-10 text-left min-h-screen">
                ...
                <div className="max-w-xl z-10 text-gray-500">
                    ...
                    <div className='flex items-center flex-wrap space-x-2 pt-3 pb-1 text-sm'>
                        <span className='font-medium text-yellow-600 mr-1'>{calculateRating(courseData).toFixed(1)}</span>
                        <div className='flex mr-1'>
                            {[...Array(5)].map((_, i) => (<img key={i} src={i < Math.round(calculateRating(courseData)) ? assets.star : assets.star_blank} alt='' className='w-3.5 h-3.5' /> ))}
                        </div>
                ...
                <div className="max-w-course-card w-full md:w-auto z-10 shadow-lg rounded-lg overflow-hidden bg-white min-w-[300px] sm:min-w-[420px] self-start md:sticky md:top-10">
                    ...
                    <div className="p-5">
                        ...
                        <div className="flex items-center justify-between text-sm md:text-default gap-4 pt-2 pb-4 text-gray-500 border-b mb-4">
                            <div className="flex items-center gap-1" title={`${calculateRating(courseData).toFixed(1)} rating`}>
                                <img src={assets.star} alt="star icon" className='w-4 h-4' /><span>{calculateRating(courseData).toFixed(1)}</span>
                            </div>
